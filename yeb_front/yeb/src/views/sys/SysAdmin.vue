<template>
    <div>
        ...
        <div class="admin-container">
            <el-card class="admin-card" v-for="(admin,index) in admins" :key="admin">
...
export default {
    ...
    methods: {
        hidePop(admin) {
            ...
            let flag = false;
            //下面的if-else是为判断原来的roles和退出时是否一样，一样的话就不需要发起put请求
        ...
        initAllRoles() {
            this.getRequest('/system/admin/roles' + this.keywords).then(resp => {
                if (resp) {
                    this.allRoles = resp;
                }
            })
        ...
        deleteAdmin(admin) {
            this.$confirm('此操作将永久删除[' + admin.name + ']角色, 是否继续?', '提示', {
